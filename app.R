...
ui <- bslib::page_navbar(
  ...
  tags$head(
    ...
    tags$link(
      rel = "stylesheet",
      type = "text/css", href = "css/highlighter.css"
    ),
    tags$link(
      rel = "stylesheet",
      type = "text/css", href = "css/fill_in_the_blanks.css?v=4"
    ),
    tags$link(
      rel = "stylesheet",
      type = "text/css", href = "css/chapter_styling.css"
    ),
  ...
  nav_panel(
    title = tagList(
      bs_icon("journal-text"),
      " Advice"
    ),
    value = "advice",
    advice_module_ui("advice")
  ),
  nav_panel(
    title = tagList(
      bs_icon("puzzle"),
...
server <- function(input, output, session) {
  ...
  advice_module_server("advice", parent_session = session, nav_order_list = nav_order_list, process_markdown = process_markdown, process_rmd_fragment = process_rmd_fragment)
