      ...
      bslib::card(
        ...
        bslib::card_body(
          ...
          div(
            ...
            actionButton(
              inputId = ns("open_highlighter_modal"),
              label = "🖊️ Learn About Using Highlighters",
              # class = "ps-action-btn"
            )
          ),
        )
...
strategies_module_server <- function(id, parent_session, nav_order_list, process_markdown, process_rmd_fragment) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$open_highlighter_modal, {
      showModal(
        modalDialog(
          title = "🖊️ Using Highlighters",
          size = "l",
          easy_close = TRUE,
          footer = modalButton("Close"),
          uiOutput(ns("highlighter_modal_ui"))
        )
      )
    })

    output$highlighter_modal_ui <- renderUI({
      tags$div(
